...
module.exports = async function handler(req, res) {
  ...
  if (!encResp) {
    console.error('[ccavenue/response] No encResp in payload.');
  ...
  try {
    ...
    console.log('[ccavenue/response] Decrypted payload:', JSON.stringify({ ...data, card_name: undefined }));
  } catch (err) {
  ...
  if (SUPABASE_URL && SERVICE_KEY && orderId) {
    ...
    try {
      ...
      if (Array.isArray(patched) && patched[0]) {
        ...
        console.warn(`[ccavenue/response] PATCH returned no rows for order_id=${orderId}. Attempting INSERT (recovery).`);
        const minimalShip = {
          name: data.billing_name || '', email: data.billing_email || '', phone: data.billing_tel || '',
          address: data.billing_address || '', city: data.billing_city || '', state: data.billing_state || '',
          zip: data.billing_zip || '', country: data.billing_country || 'India',
        };
        const insertRes = await fetch(`${SUPABASE_URL}/rest/v1/orders`, {
          method: 'POST',
          headers: { apikey: SERVICE_KEY, Authorization: `Bearer ${SERVICE_KEY}`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
          body: JSON.stringify({
            user_id: null, ccavenue_order_id: orderId, items: [],
            total_amount: Number(amount || 0), items_subtotal: Number(amount || 0),
            shipping_amount: 0, discount_amount: 0,
            guest_email: minimalShip.email, guest_phone: minimalShip.phone,
            shipping_address: minimalShip, ...commonFields,
          }),
        });
  ...
  if (dbStatus === 'Paid' && orderRow) {
    ...
    if (phone) {
      fetch(`${SITE_URL}/api/whatsapp/send`, {
        ...
        body: JSON.stringify({
          type: 'order_confirm', to: phone,
          params: {
            customer_name: name, order_id: orderId,
  ...
  const params = new URLSearchParams({
    status: ccStatus, order_id: orderId || '', tracking_id: trackingId || '', amount: amount || '',
  }).toString();
